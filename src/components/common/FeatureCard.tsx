...
function FeatureCard({ title, description, points }: FeatureCardProps) {
  return (
    <div className="relative h-full flex flex-col border border-secondary/20 p-6 md:p-8 bg-primary/40 backdrop-blur-sm">
      ...
      {points && (
        <ul className="mt-4 space-y-2 text-secondary/70 text-sm leading-relaxed list-disc list-inside">
          {points.map((point, i) => (
            <li key={i}>{point}</li>
          ))}
        </ul>
